# How to Release ligo-memory

## Prerequisites

- `git` and `gh` CLI installed and authenticated
- All changes committed and pushed to `main`
- `ligo` released first (ligo-memory depends on it)
...
```bash
go get github.com/linkeunid/ligo-memory@v0.1.1
```

## Verifying the release

```bash
GOPROXY=direct go list -m github.com/linkeunid/ligo-memory@latest
```
